...
RuleCraft::RuleCraft(const std::string &type) :
	...
	_allowLanding(true), _allowAlienBaseDetection(true), _spacecraft(false), _notifyWhenRefueled(false), _autoPatrol(false), _listOrder(0), _maxItems(0), _maxAltitude(-1), _stats(),
...
void RuleCraft::load(const YAML::Node &node, Mod *mod, int listOrder)
{
	...
	_allowAlienBaseDetection = node["allowAlienBaseDetection"].as<bool>(_allowAlienBaseDetection);
 ...
 * @return True if this ship is capable of detecting an alien base.
 */
bool RuleCraft::getAllowAlienBaseDetection() const
{
	return _allowAlienBaseDetection;
}
